...
The Starter `Refresh unified public site sources` workflow checks the three public upstream default branches hourly and compares only the public files explicitly listed in `site/sources.lock.json` and the build tool. If allowlisted bytes change, it updates the source SHA, rebuilds and validates the composed site, then commits the one-line lock change as `github-actions[bot]`; Cloudflare Workers Builds deploys the Starter `main` push to the existing `workers.dev` candidate. With no allowlisted content change, it makes no commit and triggers no deployment. It reads no files outside the allowlist, accesses no private repositories, introduces no credentials, and does not change the canonical URL, DNS, or cutover state.

Changes to the allowlist itself, path mapping, build logic, or publication policy still require a maintainer PR and full CI. Source SHAs continue to record the immutable revisions used for each actual publication; this website source lock does not replace downstream adoption locks. Scheduled checks may be delayed by GitHub Actions load. Cloudflare Workers Builds has a monthly build-minute limit on its free plan; if reached, deployments must wait for quota reset or a separately authorized plan decision. No paid upgrade is automatic.
