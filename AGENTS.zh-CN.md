...
执行任何 Cloudflare 操作前，必须说明准确目标、受影响层、数据流、凭据范围、人类批准边界、验证检查与回滚。默认 Workers Builds Profile 使用 Provider 管理的 credential；绝不要求人类把密码、token、私钥、恢复码或其他秘密发送到聊天。需要人类操作 UI 时，必须提供编号的操作者级步骤。只有显式选择高级 External-CI Profile 时才要求 Trusted Secret Broker。

Private downstream 项目不得把 GitHub Actions 当作迭代调试器。Agent 必须先批量完成相关修改、运行所有可用 Agent-side preflight、检查完整 diff，然后只触发预定的薄 CI。CI 失败时先读完整 failure set，批量修复并重新 preflight，优先只 rerun failed work。不得为了“再确认一下”给 content-only 改动制造 Actions run，也不得自动开启付费 Actions、提高 budget 或修改 billing。
