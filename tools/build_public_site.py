...
def start_page(english: bool) -> str:
    ...
    default = ("Default: full AHICP + full PPF + Vault Interface. Original sources stay private; unpublished Web stays restricted and authenticated. Ordinary new projects prefer workers-builds-native + private-project-quota-saver under the personal ChongLiuPhil account. Content-only changes do not start GitHub Actions; configuration PRs use one lightweight contract check; Cloudflare Workers Builds owns the automatic main-branch Web build; heavy GitHub workflows are manual. Allow one short human-assisted GitHub → Cloudflare bootstrap, then require a second push to deploy automatically without renewed authorization. Public release, source-repository publication, reader expansion, domains/DNS, permission expansion, paid Actions usage, and billing changes remain separate human decisions." if english else "默认完整 AHICP + 完整 PPF + Vault Interface。原创源文件默认 private；未发布 Web 默认 restricted + authenticated。普通新项目首选个人 ChongLiuPhil 账号下的 workers-builds-native + private-project-quota-saver；content-only 改动不启动 GitHub Actions，配置 PR 只跑一个轻量 contract check，main 的自动 Web build 由 Cloudflare Workers Builds 负责，heavy GitHub workflow 手动运行。允许每项目一次短人工 GitHub → Cloudflare bootstrap，并要求第二次 push 无需重新授权即可自动部署。Public release、source repository 公开、reader 扩大、domain/DNS、权限扩展、付费 Actions usage 与 billing 变化仍需人类单独决定。")
