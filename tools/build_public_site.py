...
def start_page(english: bool) -> str:
    ...
    default = ("Default: full AHICP + full PPF + Vault Interface. Original sources stay private; unpublished Web stays restricted and authenticated. Ordinary new projects prefer workers-builds-native under the personal ChongLiuPhil account. Allow one short human-assisted GitHub → Cloudflare bootstrap for the project, then require a second push to deploy automatically without renewed authorization. Public release, source-repository publication, reader expansion, domains/DNS, permission expansion, and paid-plan changes remain separate human decisions." if english else "默认完整 AHICP + 完整 PPF + Vault Interface。原创源文件默认 private；未发布 Web 默认 restricted + authenticated。普通新项目首选个人 ChongLiuPhil 账号下的 workers-builds-native；允许每项目一次短人工 GitHub → Cloudflare bootstrap，并要求第二次 push 无需重新授权即可自动部署。Public release、source repository 公开、reader 扩大、domain/DNS、权限扩展与付费变化仍需人类单独决定。")
