...
def start_page(english: bool) -> str:
    ...
    default = ("Default: full AHICP + full PPF + Vault Interface. Original sources stay private; unpublished Web stays restricted and authenticated. After a verified platform bootstrap, new-project infrastructure prefers agent-provisioned-external-ci and reuses standing authorization inside the approved scope. Public release, reader expansion, domains/DNS, permission expansion, and paid-plan changes remain separate human decisions." if english else "默认完整 AHICP + 完整 PPF + Vault Interface。原创源文件默认 private；未发布 Web 默认 restricted + authenticated。平台 bootstrap 已验证后，新项目基础设施首选 agent-provisioned-external-ci，并在已批准 scope 内复用 standing authorization。Public release、reader 扩大、domain/DNS、权限扩展与付费变化仍需人类单独决定。")
