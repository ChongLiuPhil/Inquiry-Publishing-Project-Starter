...
function appJWT(env) {
  ...
  return `${unsigned}.${sign('RSA-SHA256', Buffer.from(unsigned), createPrivateKey(env.GITHUB_APP_PRIVATE_KEY)).toString('base64url')}`;
}
...
export function makeHandler(registry, send = fetch) {
  return async (request, env) => {
    ...
    try {
      ...
      return response(502, 'Notification failed; redeliver after repair or run manual source recovery');
    }
  };
}
